...
{#if isOpen}
  ...
  >
    <div
      class="bg-(--bg-panel) border border-(--border-panel) rounded-xl shadow-2xl p-5 flex flex-col gap-4 text-(--text-base) w-full max-w-md max-h-[70vh] transition-all animate-in fade-in zoom-in-95 duration-150"
    ...
    >
      ...
      <div class="flex items-center justify-between border-b border-(--border-subtle) pb-3">
        <div class="flex items-center gap-2.5">
          ...
          <div>
            <h3 class="font-semibold text-sm text-(--text-base)">Notificações</h3>
          </div>
        </div>
      ...
      <div class="flex-1 overflow-y-auto space-y-2 pr-1 min-h-[120px] max-h-[40vh]">
        {#if notifications.length === 0}
          <div class="p-6 text-center text-xs text-(--text-muted) flex flex-col items-center gap-2">
            <span>Nenhuma notificação registrada</span>
          </div>
        {:else}
          {#each notifications as item (item.id)}
            <div class="p-2.5 rounded-lg bg-(--bg-item) border border-(--border-subtle) flex flex-col gap-1 text-xs">
              ...
              <p class="text-(--text-base) font-sans break-words">{item.message}</p>
            </div>
          {/each}
        {/if}
      </div>
