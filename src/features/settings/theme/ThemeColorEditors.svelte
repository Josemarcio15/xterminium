...
{#if showNameInput}
  <div
    class="flex items-center gap-1.5 mb-3 p-2 rounded-lg bg-(--bg-item) border border-(--btn-border)"
  >
    <span
      class="text-[10px] text-(--accent-primary) shrink-0 font-medium"
    >
    ...
    <input
      ...
      class="flex-1 min-w-0 bg-transparent border-b border-(--btn-border) text-[11px] text-(--text-base) outline-none placeholder-(--text-faint) py-0.5"
      onkeydown={(e) => {
...
<div class="flex flex-col gap-3">
  {#each themeColorGroups as group}
    <div>
      <p
        class="text-[10px] uppercase tracking-wider text-(--text-faint) font-bold mb-1.5"
      >
        {group}
      </p>
      <div class="flex flex-col gap-1">
        {#each colorFields.filter((f) => f.group === group) as field}
          <div
            class="flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-lg bg-(--bg-item) border border-(--border-subtle)"
          >
            <span class="text-[11px] text-(--text-base) font-medium">
              {field.label}
            </span>
