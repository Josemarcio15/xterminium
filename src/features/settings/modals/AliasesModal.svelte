...
<Modal {show} title="Aliases de Comandos" widthClass="w-96" {onClose}>
  ...
  {#if showForm}
    <form
      class="bg-(--bg-item) border border-white/10 rounded-md p-2.5 mb-2.5 flex flex-col gap-2"
      onsubmit={(e) => {
        e.preventDefault();
        saveAlias();
      }}
    >
      <div
        class="text-[11px] font-semibold text-amber-400 flex items-center justify-between"
      >
  ...
  <div
    class="max-h-72 overflow-y-auto flex flex-col gap-1.5 scrollbar-none [&::-webkit-scrollbar]:hidden"
  >
    {#each configStore.aliases as item (item.id)}
      <div
        class="flex items-center justify-between p-2 rounded bg-white/3 hover:bg-white/6 border border-white/5 transition-all text-xs {editingId ===
        item.id
      ...
      >
        <div class="flex flex-col min-w-0 pr-2">
          <div class="flex items-center gap-1.5 font-mono">
            ...
            <span
              class="text-emerald-400 text-[11px] bg-emerald-500/10 px-1.5 py-0.5 rounded truncate max-w-50"
              title={item.command}
            >
              {item.command}
            </span>
