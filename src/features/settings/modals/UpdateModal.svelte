<script lang="ts">
  ...
  async function startUpdateProcess() {
    errorMessage = null;
    // A política é do SO (backend): só pede senha onde o instalador usa sudo
    // (Linux/.deb). O frontend não infere o sistema por user agent.
    if (await UpdateService.needsAdminPassword()) {
      requireSudo = true;
    } else {
  ...
  async function executeInstallation() {
    ...
    try {
      ...
      errorMessage =
        typeof err === "string" ? err : err?.message || "Falha ao atualizar.";
...
{#if show}
  ...
  >
    ...
    >
      ...
      <div
        class="absolute -top-16 -right-16 w-36 h-36 bg-sky-500/15 rounded-full blur-3xl pointer-events-none"
      ></div>

      <!-- Header -->
      <div class="flex items-center gap-3">
        <div
          class="w-10 h-10 rounded-xl {requireSudo
            ? 'bg-amber-500/10 border-amber-500/20 text-amber-400'
            : 'bg-sky-500/10 border-sky-500/20 text-sky-400'} border flex items-center justify-center shrink-0"
        >
          {#if isDone}
            <svg
              width="20"
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-(--text-muted)">
            ...
            {:else}
              v{info.currentVersion} &rarr;
              <span class="text-sky-400 font-medium">v{info.latestVersion}</span
              >
            {/if}
          </p>
        </div>
      ...
      <div class="py-1 flex flex-col gap-2.5">
        {#if isUpdating}
          <div class="flex flex-col items-center justify-center py-6 gap-3">
            <div class="relative w-12 h-12">
              <div
                class="absolute inset-0 rounded-full border-2 border-sky-500/20"
              ></div>
              <div
                class="absolute inset-0 rounded-full border-2 border-sky-400 border-t-transparent animate-spin"
              ></div>
            </div>
            <p class="text-xs text-(--text-muted) animate-pulse">
              Baixando e instalando nova versão...
            </p>
          </div>
        {:else if isDone}
          <div
            class="p-3.5 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-start gap-2.5"
          >
            <svg
            ...
            <p class="text-xs text-emerald-300 font-medium leading-relaxed">
              Instalação finalizada com sucesso! Reinicie o aplicativo para
        ...
        {:else if requireSudo}
          {#if errorMessage}
            <div
              class="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-xs text-red-300 leading-relaxed"
            >
              {errorMessage}
            </div>
          {/if}
          ...
          <p class="text-xs text-(--text-muted) leading-relaxed">
            Digite sua senha de usuário (<code class="text-amber-400 font-mono"
              >sudo</code
            >) para que o sistema possa atualizar o pacote
            <strong class="text-(--text-base)">xterminium</strong>:
          </p>
        ...
        {:else}
          ...
          <p class="text-xs text-(--text-muted) leading-relaxed">
            Uma nova versão do xterminium está disponível. Deseja realizar a
            atualização automática?
          </p>

          {#if info.releaseNotes}
            <div class="flex flex-col gap-1.5 mt-1">
              <div class="flex items-center gap-1.5 text-[11px] font-semibold text-sky-400">
              ...
              <div
                class="p-3 bg-black/30 rounded-xl border border-(--border-subtle) max-h-44 overflow-y-auto text-[11px] text-(--text-base) font-mono whitespace-pre-wrap select-text leading-relaxed scroll-smooth"
              >
                {info.releaseNotes}
              </div>
            </div>
      ...
      {#if !requireSudo}
        <div
          class="flex justify-end items-center gap-2 pt-2 border-t border-(--border-subtle)"
        >
          {#if isDone}
