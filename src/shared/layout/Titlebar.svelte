  ...
  function handleStartDragging(e: MouseEvent) {
    if (
      ...
      !(e.target as HTMLElement).closest(
        "button, input, .tab, .modal-content, .actions-area",
      )
    ) {
...
>
  ...
  >
    <img
      src="/terminal.svg"
      class="w-[18px] h-[18px] block shrink-0 pointer-events-none"
      alt="xterminium Logo"
    />
    <span
  ...
  >
    <div
      data-tauri-drag-region
      style="-webkit-app-region: drag;"
      class="flex items-center gap-1 min-w-full h-full"
    >
      {#each tabs as tab (tab.id)}
        ...
        >
          ...
          >
            <span
              class="w-1.5 h-1.5 rounded-full shrink-0 {tab.type === 'ssh'
                ? 'bg-sky-400 shadow-[0_0_6px_#38bdf8]'
                : tab.type === 'sftp'
                  ? 'bg-indigo-400 shadow-[0_0_6px_#818cf8]'
                  : 'bg-[#00e699]'}"
            ></span>
            <span class="max-w-[140px] overflow-hidden text-ellipsis"
              >{tab.title}</span
            >
            <span
              ...
              onkeydown={(e) =>
                e.key === "Enter" &&
                (e.stopPropagation(),
                onCloseTab(tab.id, e as unknown as MouseEvent))}
              role="button"
              tabindex="0"
              title="Fechar aba"
            >
  ...
  <div
    style="-webkit-app-region: no-drag;"
    class="actions-area flex items-center h-full shrink-0"
  >
        ...
        >
          ...
          >
            ...
            >
              ...
              <path d="M16.5 19c1.93-1.11 3.5-2.6 3.5-4.2 0-.82-.29-1.6-.8-2.3"
              ></path>
            </svg>
    ...
    >
      <svg width="11" height="11" viewBox="0 0 12 12"
        ><polygon
          fill="currentColor"
          points="11 1.7 10.3 1 6 5.3 1.7 1 1 1.7 5.3 6 1 10.3 1.7 11 6 6.7 10.3 11 11 10.3 6.7 6"
        ></polygon></svg
      >
    </button>
  </div>
